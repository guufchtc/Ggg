...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7868379268:AAFOmeepA_2WXWTd3mP1pKowRv_enudxELM'
...
async def list_users(update, context):
    ...
    for user in users:
        ...
        if expiry_date:
            ...
            user_list_message += f"⚠️ User ID: {user_id} - Expiry Date Missing\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./venompapa {ip} {port} {duration}"
...
async def generate_redeem_code(update: Update, context: CallbackContext):
    ...
    if len(context.args) < 3:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, 
            text="*⚠️ Usage: /gen [prefix] <days/minutes> <number_of_codes> [max_uses]*", 
            parse_mode='Markdown'
        )
        return
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id, 
            text="*⚠️ Please provide a valid number of codes to generate.*", 
            parse_mode='Markdown'
        )
        return
    ...
    for _ in range(num_codes):
        ...
        # Save each code to the database
    ...
    # Send the message with the generated codes
    await context.bot.send_message(
        chat_id=update.effective_chat.id, 
        text=message, 
        parse_mode='Markdown'
